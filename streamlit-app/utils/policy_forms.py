...
def policy_manual_form(defaults=None):
    ...
    with st.form("policy_manual_form"):
        ...
        cust_id = col1.text_input("CUST_ID *", value=str(defaults.get("CUST_ID", "")), help="Mandatory")
        ...
        sum_insured = col1.text_input("SUM INSURED *", value=str(defaults.get("SUM_INSURED", "")), help="Mandatory")
        premium2 = col2.text_input("PREMIUM2 *", value=str(defaults.get("PREMIUM2", "")), help="Mandatory")
        ...
        model_year = col4.text_input("MODEL_YEAR", value=str(defaults.get("MODEL_YEAR", "")))
        regn = col5.text_input("REGN", value=defaults.get("REGN", ""))
        veh_seats = col6.text_input("VEH_SEATS", value=str(defaults.get("VEH_SEATS", "")))
...
def policy_cancel_form(defaults=None):
    ...
    with st.form("policy_cancel_form"):
        # --- Mandatory Fields in 3 columns ---
        col1, col2, col3 = st.columns(3)
        cust_id = col1.number_input("CUST_ID *", value=defaults.get("CUST_ID", 1),   format="%d", disabled=True)
        ...
        model_year = col4.number_input("MODEL_YEAR", value=defaults.get("MODEL_YEAR", 2024),   format="%d", disabled=True)
        regn = col5.text_input("REGN", value=defaults.get("REGN", ""), disabled=True)
        veh_seats = col6.number_input("VEH_SEATS", value=defaults.get("VEH_SEATS", 1),   format="%d", disabled=True)
