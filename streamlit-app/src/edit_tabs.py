...
from db_utils import insert_policy, fetch_data, insert_claim
        ...
        if ttype == "New Business":
            ...
            if not st.session_state.show_policy_summary:
                ...
                if st.button("Main menu", key="back_to_main_from_summary"):
                    _clear_new_policy_generated_keys()  # also clear when exiting summary
...
# Pre-Bind Not Implemented
        ...
        #######
            ...
            if not st.session_state.mta_policy_fetched:
                ...
                # Show inserted snapshot instead of diff update
                updated_data = st.session_state.mta_inserted_data
                ...
                if back_to_main:
                    ...
                    st.session_state.mta_policy_data = None
                    st.session_state.show_mta_summary = False
                    if "mta_inserted_data" in st.session_state:
                        del st.session_state.mta_inserted_data
                    st.rerun()
                # # Show MTA Summary
        ...
        #############
            ...
            if not st.session_state.renewal_policy_fetched:
                ...
                updated_data = st.session_state.renewal_inserted_data
        ...
        if ttype == "New Claim":
            ...
            with st.form("new_claim_form"):
                ...
                if (stored_claim_no != claim_no_current) or (f"{session_key_root}_dt" not in st.session_state):
                    ...
                    submission_dt_str = submission_dt_obj.strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]  # datetime2 format
                    
                    st.session_state[f"{session_key_root}_dt"] = submission_dt_str
                ...
                if back:
                    _clear_new_claim_generated_keys()  # Clear when going back
                    st.session_state.claims_edit_page = "main"
                    if "claim_no" in st.session_state:
                        del st.session_state.claim_no
                    st.rerun()
                ...
                with st.form("update_claim_form"):
                    ...
                    if submit:
                        try:
                            ...
                            submission_dt = datetime.now()
                            submission_dt_str = submission_dt.strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]  # datetime2 format
                            ...
                            new_row.update({
                                ...
                                "CLAIM_STAGE": "Updated",
                                # System generated fields with aligned precision
                                "Submission_Date": submission_dt_str,
                    ...
                    if submit:
                        if not confirm_closure:
                            ...
                            # Generate system metadata for closure transaction
                                ...
                                # Build new transaction row (insert new row instead of updating)
                                original = st.session_state.claim_closure_data
                                new_row = original.copy()
                                new_row.update({
                                    "FINAL_SETTLEMENT_AMOUNT": float(final_settlement),
                                    "CLAIM_CLOSURE_DATE": closure_date,
                    ...
                    if submit:
                        if not confirm_reopen:
                            ...
                            try:
                                ...
                                # Generate system metadata for reopen transaction
                                submission_dt = datetime.now()
                                submission_dt_str = submission_dt.strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]  # datetime2 format
